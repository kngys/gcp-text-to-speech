class TTSManager:
    ...
    def synthesize(self, mode, input_path, lang, voice, output_path):
        if not self.active_provider:
            raise RuntimeError("No active TTS provider selected.")
        if mode == "text":
            self.active_provider.synthesize_text(input_path, lang, voice, output_path)
        elif mode == "ssml":
            self.active_provider.synthesize_ssml(input_path, lang, voice, output_path)
        else:
